...
More technically, the Orchestrator is built as an AsyncronousWebsocket using DjangoChannels, working both as an optimizer and a workload balancer. It sends out the tasks through a secure Websocket connection to the computational nodes (which check first the trustworthyness of the Orchestrator after accessing a secure database) and then silently and actively listens for directions (run in background) on what to train and what to tune. Computational nodes send then back the result to the Orchestrator as soon as they're done, and the process is repeated iteratively either until convergence or until a stopping condition is met.
